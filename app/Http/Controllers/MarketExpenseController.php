...
class MarketExpenseController 
{
	public function create(Company $company)
	{
		$model = IncomeStatement::first();
		return view('admin.market-expenses.expense',[
			'company'=>$company , 
			'pageTitle'=>__('Marketing Expense Form'),
			'storeRoute'=>route('admin.store.market.expense',['company'=>$company->id]),
			'type'=>'create',
	...
	public function store($company_id,Request $request){
		...
		foreach((array)$request->get('tableIds') as $tableId){
			...
			foreach((array)$request->get($tableId) as  $tableDataArr){
					...
					dd($model->generateRelationDynamically($tableId) , $tableDataArr);
